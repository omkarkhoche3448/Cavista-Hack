...
def _pending_summary_payload(*, session_id: str, status: str, message: str) -> dict:
    return {
        "id": None,
        ...
        "key_takeaways": [],
        "medications_list": [],
        "follow_up_notes": "Please check again shortly.",
        "warnings": [],
        "approval_status": "pending",
        "sent_to_patient_at": None,
    }
...
def get_patient_summary(session_id: str, *, current_user: dict, supabase: Client) -> dict:
    _assert_user_participates_session(supabase, session_id, current_user["id"])
    summary = emr_repository.get_patient_summary(supabase, session_id)
    if not summary:
        return _pending_summary_payload(
            session_id=session_id,
            status="processing",
            message="Your session summary is being generated by AI.",
        )
    if current_user["role"] == "patient":
        if summary.get("approval_status") != "approved" or not summary.get("sent_to_patient_at"):
            return _pending_summary_payload(
                ...
                message="Your doctor is reviewing this summary before sharing it with you.",
            )
    return summary
